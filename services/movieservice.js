...
service.addMovie = function(movie,callback) {
        ...
        connection.query("INSERT INTO movies set ? ",movie, function(err, results) {
          if(err){
...
service.getMoviesById  = function(id,callback){
  var record = {};
...
service.updateMovie = function(movie,callback){
         pool.getConnection(function(err, connection) {
        if(err) { console.log(err); callback("fail"); return; }
        connection.query("UPDATE movies set ? WHERE id = ? ",[movie,movie.id], function(err, results) {
          if(err){
           console.log("Error Selecting : %s ",err );
           callback("fail");
          }else{
